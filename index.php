...
function is_blank_avatar(string $avatar): bool {
    return $avatar==='blank';
}
function blank_avatar_mark(): string {
    return '<svg viewBox="0 0 64 64" aria-hidden="true"><circle cx="32" cy="20" r="11" fill="currentColor"/><path fill="currentColor" d="M16 56c1.2-14 8-20 16-20s14.8 6 16 20z"/></svg>';
}
...
function placeholder_avatar_picker(string $current): string {
    ...
    $o.='<label class="avatar-pick"><input type="radio" name="avatar_choice" value="blank"'.($current==='blank'?' checked':'').'><span class="avatar-blank">'.blank_avatar_mark().'</span><span class="sr-only">Blank</span></label>';
...
function apply_avatar_choice(string $patientDir, array &$patient, string $choice): void {
    ...
    if($choice==='blank'){
        $patient['avatar']='blank';
        if(!is_blank_avatar($previous))remove_replaced_avatar($patientDir,$previous);
        return;
    }
...
function save_patient_details(): void {
    ...
    if($choice!=='keep'&&$choice!=='upload'&&$choice!=='blank'&&!in_array($choice,placeholder_avatar_files(),true))fail('Choose a portrait option.');
...
function patient_edit_dialog(array $p,string $csrf,int $revision,string $returnTo):string{
    ...
    $usingBlank=is_blank_avatar($currentAvatar);
    $pickerCurrent=$usingBlank?'blank':($usingPlaceholder?$currentAvatar:'');
...
function wound_card(array $selected,array $w,string $date,string $csrf):string{
    ...
    if($u){
        ...
        $o.='<div class="row"><button type="button" onclick="showModal(\'update-'.h($w['id']).'\')">Add update for this date</button><button type="button" onclick="showModal(\'photo-harness\')">Add Photo</button></div>';
    }
    return $o.'</article>';
}
...
function photo_harness_dialog():string{return dialog_start('photo-harness','Add Photo').'<p>This is a Harness First app. Because it is still in active development, adding photos requires the developer to prompt a photo be assigned to a particular part of the app.</p><p>Actual uploading will be coming soon.</p><div class="row"><button type="button" class="ghost" onclick="this.closest(\'dialog\').close()">Close</button></div></dialog>';}
