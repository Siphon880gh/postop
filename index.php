...
function photo_gallery(array $photos,array $w,array $l,string $date,string $csrf):string{
    ...
    foreach($photos as $i=>$p){
        ...
        $o.='<figure'.($i===0?' class="is-current"':' hidden').' data-index="'.$i.'"><img src="index.php?action=media&id='.h($p['id']).'&v='.$l['revision'].'" alt="'.h($heading).' of '.h($w['name']).'"><figcaption><span><strong>'.h($heading).'</strong>'.($p['caption']!==''?'<small>'.h($p['caption']).'</small>':'').'</span><button type="button" class="ghost small photo-expand" data-index="'.$i.'" aria-label="Expand '.h($heading).' of '.h($w['name']).'">Expand</button></figcaption></figure>';
    }
    $o.='</div>';
...
function photo_manage(array $photos,array $w,array $l,string $date,string $csrf):string{
    $n=count($photos);if($n===0)return '';
    $o='<details class="photo-manage"><summary>Manage photos</summary><ul>';
    foreach($photos as $i=>$p){
        $heading=photo_heading($p,$i);
...
function photo_add_dialog(string $csrf,array $l,array $w,string $date):string{return dialog_start('photo-add-'.h($w['id']).'-'.h($date),'Add photo to '.$w['name'].' · '.date('M j, Y',strtotime($date))).'<form method="post" enctype="multipart/form-data">'.hidden($csrf,$l['id'],$w['id'],$date).'<label>Label (optional)<input name="angle" placeholder="Angle or reshot"></label><label>Caption<input name="caption"></label><label>JPEG, PNG, or WebP (max 15 MB)<input type="file" name="photo" accept="image/jpeg,image/png,image/webp"></label><div class="row"><button type="submit" name="op" value="upload">Upload image</button><button class="ghost" type="submit" name="op" value="placeholder">Create placeholder photo</button></div></form></dialog>';}
function photo_edit_dialog(string $csrf,array $l,array $w,string $date,array $p):string{return dialog_start('photo-'.h($p['id']),'Edit photo').'<form method="post">'.hidden($csrf,$l['id'],$w['id'],$date).'<input type="hidden" name="photo_id" value="'.h($p['id']).'"><label>Label (optional)<input name="angle" value="'.h($p['angle']).'" placeholder="Angle or reshot"></label><label>Caption<input name="caption" value="'.h($p['caption']).'"></label><div class="row"><button type="submit" name="op" value="photo_edit">Save photo</button><button class="danger ghost" type="submit" name="op" value="photo_delete" onclick="return confirm(\'Delete this photo?\')">Delete photo</button></div></form></dialog>';}
