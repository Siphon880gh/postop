...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try {
      ...
      elseif($op==='count_row_save'||$op==='count_row_delete'){
        ...
        if(!isset($l['count_rows'])||!is_array($l['count_rows']))$l['count_rows']=[];
...
function day_count_phrases(array $l,string $date):array{
    ...
    $phrases=[];
    $pod=days_since((string)($l['start_date']??''),$date);
    if($pod!==null)$phrases[]='Postop '.$pod;
...
function timeline(array $l,string $date,string $view='day'):string{
    ...
    $out='<nav class="timeline" aria-label="Date timeline"><div class="month-nav"><a href="'.app_url($l['id'],$cur->modify('-1 month')->format('Y-m-d'),$view).'">Previous month</a><strong>'.$cur->format('F Y').'</strong><a href="'.app_url($l['id'],$cur->modify('+1 month')->format('Y-m-d'),$view).'">Next month</a><a href="'.app_url($l['id'],gmdate('Y-m-d'),$view).'">Today</a></div>';
    $days='';$hasNotes=false;
    for($x=$first;$x<=$last;$x=$x->modify('+1 day')){
        ...
        $pod=$postop?days_since((string)$l['start_date'],$ds):null;
        $label=$x->format('D j').($pod!==null?', Postop '.$pod:'').($photos?', '.photo_word($photos):', no photos').($nn?', '.$nn.' note'.($nn===1?'':'s'):'');
        ...
        $chip='<a aria-label="'.h($label).'" data-note-text="'.h($noteText).'"'.($current?' aria-current="date"':'').' class="date-chip '.($photos?'has-photos':'idle').' '.($current?'current':'').'" href="'.app_url($l['id'],$ds,$view).'"><small>'.$x->format('D').'</small><b>'.$x->format('j').'</b>'.($pod!==null?'<em class="pod-on-chip">Postop '.$pod.'</em>':'').($photos?'<span class="photo-mark" aria-hidden="true">'.$photos.'</span>':'').$badges.'</a>';
        if($postop){
            $stack='';
            foreach($rows as $row){
